...
export class UsersService {
  ...
  async findStaff(clinicId: string, query: any) {
    ...
    // When a branch is selected, only show staff explicitly assigned to
    // that branch (via user_branches). This is a strict isolation
    // boundary — do not weaken it. Staff who have no row in user_branches
    // yet aren't "clinic-wide"; they simply haven't been assigned to a
    // branch. Fix that from the Branches page ("Manage staff" on a branch
    // card → assigns via POST /branches/:id/staff/:userId), not by
