 ...
 * Individual blog post — wrapped in the clinic's PublicSiteLayout so the
 * full theme/nav/footer are applied, and blog content is rendered as HTML.
 */
...
async function getData(subdomain: string, slug: string) {
  const [postRes, siteRes, relatedRes] = await Promise.all([
    fetch(`${API_BASE}/api/v1/seo/${subdomain}/blog/${slug}`,          { next: { revalidate: 60 } }),
    fetch(`${API_BASE}/api/v1/website-builder/public/${subdomain}`,    { next: { revalidate: 300 } }),
    fetch(`${API_BASE}/api/v1/seo/${subdomain}/blog/${slug}/related`,  { next: { revalidate: 300 } }),
  ]);
...
export async function generateMetadata({ params }: PostPageProps): Promise<Metadata> {
  ...
  const data = await getData(subdomain, slug);
  if (!data) return { title: 'Blog Post' };

  const { postData, site } = data;
  const post   = postData.post;
  const domain = site ? getCanonicalDomain(site.website) : '';
...
export default async function BlogPostPage({ params }: PostPageProps) {
  ...
  if (postData.faqs?.length) {
    schemas.push(buildFaqSchema(postData.faqs));
  }
  ...
  for (const entry of toc) {
    processedContent = processedContent.replace(
      new RegExp(`<h([23])([^>]*)>${escapeRegex(entry.text)}</h\\1>`, 'g'),
  ...
  return (
    <>
      ...
      >
        {/* Blog post body — styled to honour the theme's text/bg colours */}
        <div
          style={{
          ...
          <div className="max-w-5xl mx-auto px-4 py-10">
            <Breadcrumb
              ...
              items={[
                ...
                { name: post.title, href: `/site/${subdomain}/blog/${slug}` },
              ]}
            />
            ...
            <div className="flex gap-10">
              ...
              <div className="flex-1 min-w-0">
                {post.categories?.[0] && (
                  <Link
                    href={`/site/${subdomain}/blog?category=${encodeURIComponent(post.categories[0])}`}
                ...
                >
                  ...
                  {(post.tags ?? []).map(tag => (
                    <Link
                      key={tag}
                      href={`/site/${subdomain}/blog?tag=${encodeURIComponent(tag)}`}
                ...
                {toc.length >= 3 && (
                  ...
                  >
                    <summary className="px-4 py-3 text-sm font-semibold cursor-pointer select-none" style={{ color: theme.textColor || '#374151' }}>
                      Contents
                    </summary>
                ...
                {/* Blog content — dangerouslySetInnerHTML preserves editor-saved HTML tags */}
                <style>{`
                ...
                {related.length > 0 && (
                  <section className="mt-12 pt-8 border-t" style={{ borderColor: theme.textColor ? `${theme.textColor}15` : '#f3f4f6' }}>
                    <h2 className="text-xl font-bold mb-5" style={{ fontFamily: theme.fontHeading, color: theme.textColor || '#111827' }}>
                      Related Articles
                    </h2>
                    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
                      {related.map((rp: RelatedPost) => (
                        <article
                          key={rp.id}
                          className="rounded-xl overflow-hidden hover:shadow-sm transition-shadow border"
                          style={{ borderColor: theme.textColor ? `${theme.textColor}15` : '#f3f4f6', background: theme.backgroundColor || '#fff' }}
                        >
                          {rp.featuredImage && (
                          ...
                          <div className="p-4">
                            <h3 className="font-semibold text-sm line-clamp-2 mb-1" style={{ color: theme.textColor || '#111827' }}>
                              <Link href={`/site/${subdomain}/blog/${rp.slug}`} className="hover:underline">
                                {rp.title}
                              </Link>
                            </h3>
                            <p className="text-xs" style={{ color: theme.textColor ? `${theme.textColor}60` : '#9ca3af' }}>
              ...
              {toc.length >= 3 && (
                <aside className="hidden lg:block w-60 flex-shrink-0">
                  <div
                    ...
                    style={{ background: theme.backgroundColor ? `${theme.backgroundColor}dd` : '#f9fafb', borderColor: theme.textColor ? `${theme.textColor}15` : '#e5e7eb' }}
                  >
                    <p className="text-xs font-semibold uppercase tracking-wide mb-3" style={{ color: theme.textColor ? `${theme.textColor}60` : '#9ca3af' }}>
                      On this page
                    </p>
                    <nav>
                      <ol className="space-y-1.5">
                        {toc.map(h => (
                          <li key={h.id} className={h.level === 3 ? 'pl-3' : ''}>
                            <a
                              href={`#${h.id}`}
                              className="text-sm hover:underline transition-colors leading-snug block"
                              style={{ color: theme.textColor ? `${theme.textColor}80` : '#6b7280' }}
                            >
                              {h.text}
                            </a>
                          </li>
